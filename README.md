...
一次真实的患者咨询通常不会停留在单轮问答：用户可能先问“头疼挂什么科”，随后追问“什么时候有号”，又临时切到“抽血在几楼”，甚至提出超出导诊范围的问题。这个项目把这条咨询旅程拆解为可控流程：意图识别 -> 槽位抽取 -> 多轮语义澄清 -> 分诊规则召回 -> 候选内语义裁决 -> 位置或排班工具查询 -> 安全回复。系统通过多轮语义澄清收集导诊所需信息，并在候选规则和结构化知识库范围内推荐科室或急诊入口；该过程不用于疾病诊断。当前版本仍是 Mock Demo / solution prototype，不代表真实医院生产系统；它展示的是如何把大模型约束在可解释、可评测、可逐步接入真实系统的业务流程内。
...
- 多轮语义澄清：在信息不足时围绕年龄、性别、主要症状、部位、发作方式和伴随表现进行有限追问，用于缩小候选科室和判断是否需要急诊入口。
...
如果用户在问科室推荐，Agent 会从多轮对话中抽取年龄、性别、孕期、主要症状等槽位，并把它们写入 State。若信息不足，`Triage Interview Planning` 会围绕年龄、性别、部位、发作方式和伴随表现生成有限追问，用于症状信息补齐和候选科室收敛；若用户已经完成科室推荐后继续问“什么时候有号”，状态机会把本轮动作切换到排班查询。

**3. 在知识库和工具范围内决策**

就诊入口路由不依赖模型自由发挥。系统先从 `triage_rules.json` 召回候选规则，再让 LLM 在候选规则内做语义裁决；位置查询调用 `search_location`，排班查询调用 `get_doctor_schedule`。所有决策都被限制在 Mock Data 和工具返回结果范围内。
...
flowchart TB
    ...
    subgraph GRAPH["LangGraph StateGraph：Guardrailed Hospital Routing Agent"]
        ...
        ROUTE{"action 条件路由"}
        TRIAGE["extract_triage<br/>规则召回 / 候选内裁决 / Triage Interview Planning"]
...
- `Triage Interview Planning`：`extract_triage` 内部的追问规划逻辑，用于决定“继续追问”还是“进入科室推荐”；它只做多轮语义澄清、症状信息补齐、候选科室收敛和就诊入口路由，不用于判断具体疾病。
