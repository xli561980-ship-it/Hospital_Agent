...
Hospital Guide Agent 是一个面向医院门诊服务中心、互联网医院和院内客服场景的 **Guardrailed Hospital Routing Agent** 原型。它不是通用聊天机器人，也不是医疗诊断系统，而是围绕“患者问诊前分流、院内位置指引、挂号路径辅助、排班/号源查询、风险边界控制”这一组具体门诊服务流程设计的垂直行业 Agent。

一次真实的患者咨询通常不会停留在单轮问答：用户可能先问“头疼挂什么科”，随后追问“什么时候有号”，又临时切到“抽血在几楼”，甚至询问诊断和用药。这个项目把这条咨询旅程拆解为可控流程：意图识别 -> 槽位抽取 -> 分诊规则召回 -> 候选内语义裁决 -> 位置或排班工具查询 -> 安全回复。当前版本仍是 Mock Demo / solution prototype，不代表真实医院生产系统；它展示的是如何把大模型约束在可解释、可评测、可逐步接入真实系统的业务流程内。
...
- 医疗场景存在明确风险边界，不能让大模型自由输出诊断、治疗或用药建议。
...
如果用户在问科室推荐，Agent 会从多轮对话中抽取年龄、性别、孕期、主要症状等槽位，并把它们写入 State。若信息不足，系统只围绕导诊所需字段追问；若用户已经完成科室推荐后继续问“什么时候有号”，状态机会把本轮动作切换到排班查询。

**3. 在知识库和工具范围内决策**

分诊决策不依赖模型自由发挥。系统先从 `triage_rules.json` 召回候选规则，再让 LLM 在候选规则内做语义裁决；位置查询调用 `search_location`，排班查询调用 `get_doctor_schedule`。所有决策都被限制在 Mock Data 和工具返回结果范围内。

**4. 生成有边界的导诊回复**

最终回复只基于结构化上下文生成，例如推荐科室、急诊入口提示、位置路线、可用排班或拒答说明。医疗安全护栏会拦截诊断、治疗、用药和风险夸大表达；命中急诊入口时只提示前往急诊分诊台，不输出普通号源建议。

| 流程环节 | 技术组件 | 当前实现 |
|---|---|---|
| 识别用户目标 | Intent Classification | `classify_intent`，LLM + 规则回退 |
| 形成结构化上下文 | State Transition / Slot Extraction | `ingest_and_transition`，多轮 State 合并 |
...
flowchart TB
    ...
    subgraph GRAPH["LangGraph StateGraph：Guardrailed Hospital Routing Agent"]
        ...
        ROUTE{"action 条件路由"}
        TRIAGE["extract_triage<br/>规则召回 / 候选内裁决"]
...
以下为示例形态，具体科室、医生、房间和排班以当前 Mock Data 返回为准。

**普通导诊**

```text
用户：我25岁女，头疼挂什么科？
Agent：推荐科室：以当前 Mock Data 命中的导诊规则为准。
Agent：如需挂号，可继续询问今天或接下来几天的号源。
```
...
```text
用户：抽血在几楼？
Agent：位置：以当前 Mock Data 返回的楼栋、楼层和房间为准。
Agent：路线：以位置知识库中的 directions 字段为准。
```
...
Agent：推荐科室：以当前 Mock Data 命中的导诊规则为准。
用户：那什么时候有号？
Agent：号源：沿用上一轮推荐科室，查询当前 Mock Data 中的可用排班。
```
...
```text
用户：我是不是脑梗？吃什么药？
Agent：我可以协助科室推荐、院内指引与挂号/号源查询，但不能进行疾病诊断或提供用药/治疗建议。
```

展示点：不做诊断、不做治疗、不做用药建议、不判断病情严重程度。
...
- 需要医疗安全边界：同一句话可能包含症状、诊断诉求和用药诉求，系统必须限制输出范围。
