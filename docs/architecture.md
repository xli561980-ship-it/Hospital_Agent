...
- 展示系统如何通过多轮语义澄清收集导诊所需信息，并在候选规则和结构化知识库范围内推荐科室或急诊入口；该过程不用于疾病诊断。
...
flowchart TB
    ...
    subgraph GRAPH["LangGraph StateGraph：Guardrailed Hospital Routing Agent"]
        ...
        subgraph TRIAGE_MODULE["extract_triage：Triage Engine"]
            ...
            INTERVIEW["Triage Interview Planning<br/>多轮语义澄清 / 追问规划"]
...
| `Triage Interview Planning` | `extract_triage` 内部模块，决定继续追问还是进入科室推荐 | 已有槽位、候选规则、多轮上下文 | `triage_followup_questions`, `triage_candidate_departments`, `triage_positive_findings`, `triage_negative_findings` |
...
- LLM 在候选规则内做语义裁决。
- `Triage Interview Planning`：当槽位不足或候选科室仍然分散时，生成一个有限追问，用于补齐导诊信息和收敛候选科室；它不是完整临床问询流程，也不是判断具体疾病的模块。
...
5. 若为导诊，系统抽取槽位并召回候选规则；信息不足时进入 `Triage Interview Planning` 做多轮语义澄清；若为位置或排班，调用对应工具。
